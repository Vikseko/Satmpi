...
int main(int argc, char **argv) {
    ...
    ofstream out;
    const string MAPLEPATH = "./Maplemod/maple";
    ...
    string nameM = "mapleout" + to_string(rank);
    ...
    for (int z=0;z<iterations;z++){ //собственно цикл итераций
    	printf("%d", rank);
    	...
    	string scmd = MAPLEPATH + " " + var_decay + " -cpu-lim=" + to_string(timecpu) + " MD4_implication_vars_16.cnf > " + nameM;
		...
		if (rank == 0){ //главный процесс соединяет все базы с повторяющимися
    		...
    		//полученную базу в повторами каждого процесса проверяем на повторы
    		//ещё раз, тем самым оставляя только повторы между процессами
    		system("cat MD4_implication_vars_16.cnf repeatsrepeats > cnfwithrepeats.cnf");
		}

		if (rank == 0){ //главный процесс соединяет все базы с уникальными
						//дизъюнктами разных процессов в одну
			string creation = "cat";
			for ( int i = 0; i<size; i++) {
		...
		if (rank == 0){ //главный процессор удаляет лишнее и
						//переименовывает текущее, чтобы подготовиться
						//к новой итерации
			system("./FindingRep.py nameunic > repeatnameunic");
			//из файла с уникальными дизъюнктами разных процессов берём те, что
			//повторяются
			//соответственно это будут повторяющиеся между процессами
			system("cat MD4_implication_vars_16.cnf repeatnameunic > CNFwithUnic.cnf");
			//создание новой оригинальной кнф с уникальными дизъюнктами
			remove("freq_file");
			remove("stat");
			remove("split_stat");
			remove("str_len_file");
			...
			for ( int i = 0; i<size; i++) { //в процессе работы создаются куча
											//файлов, этот цикл стирает лишние
				string namehere1 = "hashout" + to_string(i);
				string namehere2 = "hashmodout" + to_string(i);
				string namehere7 = "hashmodoutun" + to_string(i);
				string namehere3 = "mapleout" + to_string(i);
			...
			remove("repeats");
			remove("nameunic");
			if (unrep==1){  //если флаг анреп был 1, то итерации идут по
							//уникальным дизъюнктам
			...
			if (unrep==0){  //если флаг анреп был 0, то итерации идут
							//по повторяющимся дизъюнктам
